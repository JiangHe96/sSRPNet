...
parser.add_argument("--U_ite", type=int, default=550, help="opt.U_ite")
parser.add_argument("--D_ite", type=int, default=200, help="opt.D_ite")
...
def Upscale_selftraining(msi,msi_down,pan_down, model,criterion,iterationmax,logger):
    ...
    for iteration in range(iterationmax):
        ...
        with torch.no_grad():
            output_test = model(msi, pan)
            if iteration>800 and iteration % 10 == 9:
                torch2tiff(output_test, outfile + str(iteration)+'IFN_UpscaleST.tiff')
            index = analysis_accu(GT[0, :, :, :], output_test[0, :, :, :], ratio)
        logger.info(" Iter[%06d], learning rate : %.9f, Train Loss: %.6f, Test: %.4f, %.4f, %.4f, %.4f, %.4f, %.4f " % (
            iteration, optimizer.param_groups[0]['lr'], temp.numpy(), index[0, 0].data, index[1, 0].data,
            index[2, 0].data, index[3, 0].data, index[4, 0].data, index[5, 0].data))
...
IFN=TFNet(channel_msi,channel_pan,ratio=ratio)
sSRHN=sSRHelp(channel_msi, channel_pan)

star_time=time.time()
Upscale_selftraining(msi,msi_down,pan_down,IFN,Upscale_criterion,opt.U_ite,logger_U)
with torch.no_grad():
    IniFus=IniFused(msi,pan, IFN)
print("Time for IFN_UpscaleST is {:6f}".format(time.time()-star_time))

analysis(GT,IniFus,ratio)
